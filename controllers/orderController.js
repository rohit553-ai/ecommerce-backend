...
orderController.newOrder = async (req, res, next) => {
  const { products, subTotal, estimatedTax, estimatedTotal, deliveryAddress } =
    req.body;
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      if (product.quantity == 0 || product.quantity < products[i].quantity) {
        throw new CustomError(
          `${product.name} is out of stock or less in stock`,
          400
        );
      }
      ...
      await productService.updateWithTransaction(
        { id: products[i].id },
        dataToUpdate,
        t
      );
...
orderController.orderWithKhalti = async (req, res, next) => {
  ...
  const result = await sequelize.transaction(async (t) => {
    ...
    if (response.status !== "success") {
      throw new CustomError(
        "Failed to verify khalti transaction, Please try ordering again",
        400
      );
    }
    return order;
...
orderController.getAllOrders = async (req, res, next) => {
  const pageLimit =
    req.query && req.query.limit ? Number(req.query.limit) : null;
...
orderController.cancelOrder = async (req, res, next) => {
  ...
  if (order.status !== "pending") {
    return next(
      new CustomError(
        "The order is already in progress and cannot be cancelled now",
        400
      )
    );
  }
